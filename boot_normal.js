...
// payload EKLENMEDİ: normal mod testi
...
(async()=>{
  ...
  chk('player modu KAPALI (normal açılış)', !body.classList.contains('player-mode'));
  const pn=document.getElementById('projectNameInput');
  chk('proje adı alanı kuruldu', !!pn, pn&&pn.value);
  chk('editör oluşturuldu', !!document.getElementById('editorHost'));
  chk('kod sekmesi render edildi', (document.getElementById('codeTabs')||{children:[]}).children.length>0,
      ((document.getElementById('codeTabs')||{children:[]}).children.length)+' sekme');
  chk('palet öğeleri yerinde', documentEl.querySelectorAll('.palette-item').length===38,
      documentEl.querySelectorAll('.palette-item').length+' öğe');
  chk('araç kutusu düğmesi var', !!document.getElementById('btnToolbox'));
  chk('player dışa aktarma düğmesi var', !!document.getElementById('btnExportPlayer'));
  chk('dil düğmesi var', !!document.getElementById('btnLang'));
  ...
  chk('varsayılan bileşenler kuruldu (LED+buton)', comps.length===2, comps.length+' bileşen');
  chk('LED bileşeni var', documentEl.querySelectorAll('.comp-led').length>0);
  chk('buton bileşeni var', documentEl.querySelectorAll('.comp-btn').length>0);
  const status=document.getElementById('statusPill');
  chk('durum "Hazır" (kendiliğinden çalışmadı)', status && status.textContent==='Hazır', status&&status.textContent);
  ...
  chk('bileşen etiketleri yazıldı', names.join(',')==='LED1,BUTON1', names.join(','));

  // Program bileşenleri GERÇEKTEN etkiledi mi?
  // Kod: YAK LED1 / BEKLE / SONDUR LED1 / YAK LED2 -> sonunda LED2 yanık, LED1 sönük
  chk('otomatik kayıt ÇALIŞTI (normal mod)', Object.keys(store).some(k=>k.indexOf('autosave')>=0),
